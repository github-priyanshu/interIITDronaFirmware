...
#define MSP_SET_FAILSAFE_CONFIG      76    // in message          Sets FC Fail-Safe settingsf
...
#define MSP_MAX_ALT                 123    // out message         Returns max altitude in althold mode
#define MSP_COMMAND                 124    // out message         Commands status
...
#define MSP_SET_COMMAND             217    // commands from app
#define MSP_SET_MAX_ALT             218    // max allowed altitude in althold mode
// #define MSP_BIND                 240    //in message          no param

// app data packets
#define MSP_APP_HEADING   221

#define MSP_APP_GPS       222

#define MSP_EEPROM_WRITE  250    // in message          no param

#define MSP_DEBUGMSG      253    // out message         debug string buffer
#define MSP_DEBUG         254    // out message         debug1,debug2,debug3,debug4
#define MSP_FLIGHT_STATUS 255    // out message         Send FlightStatusIndicatorFlag
...
// FIXME remove ;'s
static const box_t boxes [ CHECKBOX_ITEM_COUNT + 1 ] = { { BOXARM, "ARM;", 0 }, { BOXANGLE, "ANGLE;", 1 }, { BOXHORIZON, "HORIZON;", 2 }, { BOXBARO, "BARO;", 3 },
                                                         //{ BOXVARIO, "VARIO;", 4 },
                                                         { BOXMAG, "MAG;", 5 },
...
void mspInit ( serialConfig_t *serialConfig ) {
  ...
  mspPort_t *candidatePort = &mspPorts [ 0 ];

  mspSerialPort = candidatePort->port;
...
static bool processOutCommand ( uint8_t cmdMSP ) {
  ...
  switch ( cmdMSP ) {
    ...
    case MSP_BOARD_INFO:
      headSerialReply ( 2 + BOARD_IDENTIFIER_LENGTH + BOARD_HARDWARE_REVISION_LENGTH );
      serialize16 ( BOARD_IDENTIFIER_LENGTH );
    ...
    case MSP_BUILD_INFO:
      headSerialReply (
                BUILD_DATE_LENGTH + BUILD_TIME_LENGTH + GIT_SHORT_REVISION_LENGTH );
    ...
    case MSP_STATUS:
      headSerialReply ( 11 );
      serialize16 ( ( uint16_t ) flightIndicatorFlag );
    ...
    case MSP_ATTITUDE:
      ...
      serialize16 ( ( inclination_generalised.values.rollDeciDegrees ) );

      serialize16 ( ( inclination_generalised.values.pitchDeciDegrees ) );
    ...
    case MSP_ADJUSTMENT_RANGES:
      headSerialReply ( MAX_ADJUSTMENT_RANGE_COUNT * ( 1 +    // adjustment index/slot
                                                       1 +    // aux channel index
                                                       1 +    // start step
                                                       1 +    // end step
    ...
    case MSP_CF_SERIAL_CONFIG:
      headSerialReply ( ( ( sizeof ( uint8_t ) + sizeof ( uint16_t ) + ( sizeof ( uint8_t ) * 4 ) ) * serialGetAvailablePortCount ( ) ) );
    ...
    case MSP_APP_HEADING:
      appHeading = read16 ( );
      break;

    case MSP_APP_GPS:
      app_GPS_coord [ LAT ] = read32 ( );
      app_GPS_coord [ LON ] = read32 ( );
      break;

    case MSP_MAX_ALT:
      headSerialReply ( 2 );
      serialize16 ( max_altitude );

      break;

    case MSP_COMMAND:
      headSerialReply ( 3 );
      serialize16 ( current_command );
      serialize8 ( command_status );
...
static bool processInCommand ( void ) {
  ...
  switch ( currentPort->cmdMSP ) {
    ...
    case MSP_SET_PID:
      ...
      // updateTimeConstantandGains( currentProfile->pidProfile.D8[PIDALT]);
    ...
    case MSP_SET_MISC:
      ...
      masterConfig.gpsConfig.provider = ( gpsProvider_e ) read8 ( );    // gps_type
      read8 ( );                                                        // gps_baudrate
      masterConfig.gpsConfig.sbasMode = ( sbasMode_e ) read8 ( );       // gps_ubx_sbas
    ...
    case MSP_SET_CF_SERIAL_CONFIG: {
      ...
      while ( remainingPortsInPacket-- ) {
        ...
        serialPortConfig_t *portConfig = serialFindPortConfiguration ( ( serialPortIdentifier_e ) identifier );
        if ( ! portConfig ) {
          headSerialError ( 0 );
          break;
        }

        portConfig->identifier              = ( serialPortIdentifier_e ) identifier;
...
bool mspSerialRxBytesWaiting ( void ) {
  ...
  return serialRxBytesWaiting ( mspSerialPort );
}

uint8_t mspSerialRead ( void ) {

  return serialRead ( mspSerialPort );
}
